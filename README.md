...
The AI button appears only when the entered text cannot be read as a plain number list. The AI route accepts POST `/api/parse` with JSON `{ "input": "..." }` (up to 2,000 characters). It uses `@cf/meta/llama-3.1-8b-instruct`, validates decimal strings in the returned list, and never computes the total. Do not send sensitive text to this optional endpoint. When AI quota is exhausted, the button reports the error and regular local calculation remains available.
